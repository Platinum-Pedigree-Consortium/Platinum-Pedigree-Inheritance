...
def get_candidates(ivec, tr_by_sample):
...
def eval_assignment(ivec, tr_by_sample, assign):
    ...
    for sample, haps in ivec.items():
        ...
        observed = tr_by_sample[sample].alleles
...
def get_best_assign(ivec, tr_by_sample, allele_candidates_by_hap):
    ...
    for rec in itertools.product(*allele_candidates):
        assign = Assign(*rec)
        hap_to_dists = eval_assignment(ivec, tr_by_sample, assign)
